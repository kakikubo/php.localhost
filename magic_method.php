...
class SomeClass
{
		private $values = array(); // privateな値を保持するコンテナ
...
//var_dump($obj->foo);
echo "Call isset", PHP_EOL;
var_dump(isset($obj->foo)); // __isset()が呼ばれる isset: foo, true
echo "Call empty", PHP_EOL;
var_dump(empty($obj->foo)); // __isset(),__get()が呼ばれる isset: foo, get: foo, false
echo "Call unset", PHP_EOL;
unset($obj->foo);
var_dump(isset($obj->foo));
echo "Call",PHP_EOL;
$obj->bar('baz');
echo "Call Static",PHP_EOL;
